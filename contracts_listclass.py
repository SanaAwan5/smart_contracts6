...
class Gradient_List():
    ...
    def generate_gradient_avg(self, alpha=1):
        length = len(self)
        self.gradient_list[0].grad_values = self.gradient_list[0].grad_values * alpha;
        for i in range(1, length):
            self.gradient_list[0].grad_values += self.gradient_list[i].grad_values * alpha
        avg_grad_values = self.gradient_list[0].grad_values/length
...
class Model():
    ...
    def evaluate_gradient_from_avg(self,addr,avg, prikey, pubkey, inputs, targets, alpha=1):
        avg_gradient_values = avg
        candidate = copy.deepcopy(self.syft_obj)
        candidate.weights -= avg_gradient_values
        candidate.weights = candidate.weights.decrypt(prikey)
        candidate.encrypted = False
